$(".PcMeW").change(function(){
	
	var bool=$("input[name='multi_room_type']:checked").length;	
	var val="";	
	
	for(var i=0;i<bool;i++){		
		val += $("input[name='multi_room_type']:checked").eq(i).val()+",";
	}	
	var shopItemType1= val.substring(0,val.length-1);
	console.log("val : "+val); // 글자
	console.log("shopItemType1 : "+shopItemType1); //타입
	console.log("bool : "+bool); //길이	
	var mapView=$(".fxRDHg");
	mapView.html("");
	var str="";
	if(bool==1){
	...
	geocoder.addressSearch(shopItemAddr1, function(result, status) {
	 ...
	 $.ajax({		
			...
			if(data.list!=""){
				var msg="";									
				for(var i=0;i<data.list.length;i++){
					console.log(data[i].list.shopItemDealType);
					var hi=data[i].shopItemDealType;
					if(hi=="월세"){
						msg=data[i].list.shopItemDeposit+"/"+data[i].list.shopItemDealPrice;
					}else{
						msg=data[i].list.shopItemDealPrice;
					}					
					str+="<li class='Mmhsh'>";
					str+="<div class='styled__Card-fi3k4t-0 OUJOU'>";
					str+="<div class='styled__BtnWrap-sc-3yrk4m-0 gYMri'>";
