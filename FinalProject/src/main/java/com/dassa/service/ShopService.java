...
public class ShopService {
	 ...
	 * @throws Exception
	 */
	public ShopItemPageDataVO selectAllList(int reqPage,UserVO uservo, String shopItemManager) throws Exception{
		//페이지 당 게시물 수
		Map<String, Object> map = new HashMap<String, Object>();
		 map.put("shopItemManager", shopItemManager);
		int numPerPage = 5;
		//총 게시물 수 구하기
		int totalCount = shopMapper.shopItemTotalCount(map);
		//총 페이지 수 구하기
		...
		if(pageNo !=1) {
			...
			pageNavi += "<a href='/shop/item?shopItemManager="+shopItemManager+"&reqPage="+(pageNo-1)+"'></a>";
			pageNavi += "</li>";
		}
		...
		while( !(i++>pageNaviSize || pageNo>totalPage) ) { //둘 중 하나라도 만족하면 수행하지 않겠다
			if(reqPage == pageNo) {
				...
				pageNavi += "<li class=''>";
				pageNavi += "<a href='/shop/item?shopItemManager="+shopItemManager+"&reqPage="+pageNo+"'>"+pageNo+"</a>";
				pageNavi += "</li>";
			}
			pageNo++;
		}
		...
		if(pageNo <= totalPage) {
			...
			pageNavi +="<a href='/shop/item?shopItemManager="+shopItemManager+"&reqPage="+pageNo+"'></a>";
			pageNavi += "</li>";
	 ...
	 * @throws Exception
	 */
	public ShopReservationPageDataVO selectReservationAllList(int reqPage,UserVO uservo, String shopItemManager) throws Exception{
		...
		int numPerPage = 5;
		//총 게시물 수 구하기
		int totalCount = shopMapper.shopReservationTotalCount(map);
		//총 페이지 수 구하기
		int totalPage = (totalCount%numPerPage==0)?(totalCount/numPerPage):(totalCount/numPerPage)+1;
		//요청 페이지의 시작 게시물 번호와 끝 게시물 번호 구하기
		//시작 게시물 번호
		...
		if(pageNo !=1) {
			pageNavi += "<li class='prev arrow'>";
			pageNavi += "<a href='/shop/reserve?shopItemManager="+shopItemManager+"&reqPage="+(pageNo-1)+"'></a>";
			pageNavi += "</li>";
		}
		//페이지 번호 버튼 생성 ( 1 2 3 4 5 )
		int i = 1;
		while( !(i++>pageNaviSize || pageNo>totalPage) ) { //둘 중 하나라도 만족하면 수행하지 않겠다
			if(reqPage == pageNo) {
				...
				pageNavi += "<li class=''>";
				pageNavi += "<a href='/shop/reserve?shopItemManager="+shopItemManager+"&reqPage="+pageNo+"'>"+pageNo+"</a>";
				pageNavi += "</li>";
			}
			pageNo++;
		}
		...
		if(pageNo <= totalPage) {
			pageNavi += "<li class='next arrow'>";
			pageNavi +="<a href='/shop/reserve?shopItemManager="+shopItemManager+"&reqPage="+pageNo+"'></a>";
			pageNavi += "</li>";
		}
