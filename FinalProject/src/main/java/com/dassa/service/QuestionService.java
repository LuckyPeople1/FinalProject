...
public class QuestionService {
	...
	public QuestionPageData selectAllList(int reqPage, int code) throws Exception {
		...
		if (pageNo != 1) {
			if(code==1) {
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "&code=1'>이전</a>";
			}else if(code==2){
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "&code=2'>이전</a>";
			}else {				
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "'>이전</a>";
			}
		}
		...
		while (!(i++ > pageNaviSize || pageNo > totalPage)) { // 둘 중 하나라도 만족하면 수행하지 않겠다
			if (reqPage == pageNo) {
				...
				if(code==1) {
					pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "&code=1'>" + pageNo + "</a>";
				}else if(code==2) {
					pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "&code=2'>" + pageNo + "</a>";
				}else {					
					pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "'>" + pageNo + "</a>";
				}
			}
			pageNo++;
		}
		...
		if (pageNo <= totalPage) {
			if(code==1) {
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "&code=1'>다음</a>";
			}else if(code==2) {
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "&code=2'>다음</a>";
			}else {				
				pageNavi += "<a class='pbtn' href='/manage/board/question/questionManageList?reqPage=" + pageNo + "'>다음</a>";
			}
		}
