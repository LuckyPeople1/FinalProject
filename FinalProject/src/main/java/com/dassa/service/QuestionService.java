...
public class QuestionService {
	...
	public QuestionPageData selectAllList(int reqPage, int code) throws Exception {
		...
		if (pageNo != 1) {
			if(code==1) {
				pageNavi += "<a class='btn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "&code=1'>이전</a>";
			}else if(code==2){
				pageNavi += "<a class='btn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "&code=2'>이전</a>";
			}else {				
				pageNavi += "<a class='btn' href='/manage/board/question/questionManageList?reqPage=" + (pageNo - 1) + "'>이전</a>";
			}
		}
